...
class OptimizationStrategyExtractor:
    ...
    def process_file(self, file_content: str, excel_path: str) -> Tuple[Dict[str, Any], str, pd.DataFrame]:
        ...
        try:
            ...
            # 检查是否为"无明显问题"情况
            has_no_obvious_problem = any("无明显问题" in cause for cause in question_causes)
            ...
            if has_no_obvious_problem:
                ...
                json_result = {
                    'image_optimization_strategies': [{
                        ...
                        "问题归因及优化策略": [{
                            "问题归因": "未识别到具体问题",
                            "优化策略列表": [],
                            "备注": "街道无需进一步提升，没有相关的优化策略参考"
                        }]
                    }]
                }
                
                # 生成Markdown结果
                markdown_result = f"# 问题归因及优化策略\n\n## {image_name}\n\n该街道断面问题归因结果分别关联的优化策略如下：街道无需进一步提升，没有相关的优化策略参考\n"
                ...
                excel_result_df = pd.DataFrame([{
                    ...
                    '优化策略': '',
                    '优化策略内涵': '',
                    'markdown结果': '该街道断面问题归因结果分别关联的优化策略如下：街道无需进一步提升，没有相关的优化策略参考'
                }])
                
                return json_result, markdown_result, excel_result_df
            
            # 正常情况：有问题归因结果，需要匹配优化策略
            # 步骤4：加载策略映射
            ...
            # 检查是否所有策略列表都为空
            all_empty = all(len(strategies) == 0 for strategies in unique_causes.values())
    ...
    def process_jsonl_file(self, file_path: str, excel_path: str, show_progress: bool = True) -> Tuple[Dict[str, Any], str, pd.DataFrame]:
        ...
        try:
            ...
            for image_name, results in image_results.items():
                # 检查是否为"无明显问题"情况（现在统一使用"未识别到具体问题"）
                has_no_obvious_problem = any("未识别到具体问题" in result['问题归因'] for result in results) or any("无明显问题" in result['问题归因'] for result in results)
                    ...
                    if all_empty:
                        ...
                        image_json_result = {
                            ...
                            "问题归因及优化策略": [{
                                "问题归因": "未识别到具体问题",
                                "优化策略列表": [],
                                "备注": "该街道断面问题归因结果分别关联的优化策略如下：街道无需进一步提升，没有相关的优化策略/实施举措参考"
                            }]
                        }
                        
                        # 构建图片的Markdown结果
                        image_markdown = f"## {image_name}\n\n该街道断面问题归因结果分别关联的优化策略如下：街道无需进一步提升，没有相关的优化策略/实施举措参考\n\n"
                        ...
                        excel_results.append({
                            ...
                            '优化策略': '',
                            '优化策略内涵': '',
                            'markdown结果': '该街道断面问题归因结果分别关联的优化策略如下：街道无需进一步提升，没有相关的优化策略/实施举措参考'
                        })
                    else:
                        # 正常情况：有非空策略列表
                        # 构建图片的JSON结果
    ...
    def process_excel_file(self, file_content: bytes, excel_path: str, image_basename_col: str, question_causes_col: str, show_progress: bool = True) -> Tuple[Dict[str, Any], str, pd.DataFrame]:
        """
                ...
                if has_no_obvious_problem:
                    ...
                    image_json_result = {
                        ...
                        "问题归因及优化策略": [{
                            "问题归因": "未识别到具体问题",
                            "优化策略列表": [],
                            "备注": "该街道断面问题归因结果分别关联的优化策略如下：街道无需进一步提升，没有相关的优化策略参考"
                        }]
                    }
